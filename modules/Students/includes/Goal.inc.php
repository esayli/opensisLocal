...
#echo isset($edit_per_prnt[1]['CAN_EDIT']);
	...
	{	
		if(((User('PROFILE') == 'admin') && isset($edit_per_adm[1]['CAN_EDIT'])) || ((User('PROFILE') == 'teacher') && isset($edit_per_teach[1]['CAN_EDIT'])) || ((User('PROFILE') == 'student') && isset($edit_per_stu[1]['CAN_EDIT'])) || ((User('PROFILE') == 'parent') && isset($edit_per_prnt[1]['CAN_EDIT'])))
		{
			...
			$header .= '<TR>';
			$header .= '<TD>Task Title</TD><TD>' . SelectInput($RET['GOAL_ID']?$RET['GOAL_ID']:$_REQUEST['goal_id'],'tables[progress]['.$_REQUEST['progress_id'].'][GOAL_ID]','',$options,false) . '</TD>'; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
...
$edit_per_prnt = DBGet(DBQuery('SELECT CAN_EDIT FROM profile_exceptions WHERE MODNAME=\'Students/Student.php&category_id=5\' AND PROFILE_ID=3'));
	...
	{	
		if(((User('PROFILE') == 'admin') && isset($edit_per_adm[1]['CAN_EDIT'])) || ((User('PROFILE') == 'teacher') && isset($edit_per_teach[1]['CAN_EDIT'])) || ((User('PROFILE') == 'student') && isset($edit_per_stu[1]['CAN_EDIT'])) || ((User('PROFILE') == 'parent') && isset($edit_per_prnt[1]['CAN_EDIT'])))
		{
			...
			$header .= '<TR>';
			$header .= '<TD>Task Title </TD><TD>' . TextInput($RET['GOAL_TITLE'],'tables[goal]['.$_REQUEST['goal_id'].'][GOAL_TITLE]','','size=75 maxlength=50') . '</TD>';
			$header .= '</TR>';
			$header .= '<TR>';
			...
			$header .= '</TR>';
			$header .= '<TR>';
			$header .= '<TD valign=top>Task Description</TD><TD>' . TextAreaInput($RET['GOAL_DESCRIPTION'],'tables[goal]['.$_REQUEST['goal_id'].'][GOAL_DESCRIPTION]','', 'rows=10 cols=70', 'true', '200px') . '<input type="hidden" name="tabl" id="tabl" value="goal"></TD>';
			$header .= '</TR>';
			$header .= '</TABLE>';
			DrawHeader($header);
		}
	...
	DrawHeaderHome('PBL Project Info',"<A HREF=for_window.php?modname=Students/Student.php&include=Goal&modfunc=$_REQUEST[modfunc]&course_modfunc=search>Search</A>");
	...
	echo '<TD valign=top>';
	$columns = array('GOAL_TITLE'=>'PBL Project Info');
	$link = array();
	...
	ListOutput($subjects_RET,$columns,'Goal','PBL Project Info',$link,array(),$LO_options);
	echo '</TD>';
